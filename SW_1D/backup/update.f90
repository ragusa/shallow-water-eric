...
CONTAINS
  ...
  SUBROUTINE mSGN_RHS(un,rk)
    ...
    DO n = 1,mesh%np
      ...
      !eta(n) = un(inputs%syst_size-1,n) * one_over_h(n)

      ! this is pTilde and source term
      IF (eta(n) > 1.d-14) THEN
        pTilde(n) = paper_constant(n)/3.d0 * eta(n) * (h(n)-eta(n))  * (h(n) + eta(n))
        s(n) = -paper_constant(n) * (h(n) - 3.d0 * eta(n)) * (h(n)-eta(n))
      ELSE
       pTilde(n) = 0.d0
       s(n) = -paper_constant(n) * (2.d0 * heta(n))
      END IF
    END DO
